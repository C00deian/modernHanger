import React from 'react'
import Dancer0 from '../../assets/Sports/Dancer/4.jpg'
import Dancer1 from '../../assets/Sports/Dancer/2.jpg'
import Dancer2 from '../../assets/Sports/Dancer/3.jpg'
import Dancer3 from '../../assets/Sports/Dancer/1.jpg'
import ProductDisplay from "../../ProductDisplay";

function Dancer() {
    const dancer = {
        title: "Blueeagle Medal Hanger",
        brand: "Generic",
        price: "₹ 598.00",
        taxInfo: "Inclusive of all taxes",
        emiInfo: "EMI starts at ₹109 per month. EMI options",
        images: [
            Dancer1,
            Dancer0,
            Dancer2,
            Dancer3],
        details: [
            "Colour: Black",
            "Brand: Generic",
            "Material: Metal",
            "Finish Type: Metallic",
            "Shape: Rectangular",
            "Included Components: Metal",
        ],
        about: [
            "Black in color",
            "Stylish Medal Holder Display: Use this high-quality medal holder...",
            "Functional and Versatile: Our medal holder is made to be both versatile...",
            "It's simple to set up your medal display with secure display and easy installation...",
        ],
    };
    return (
        <>
            <ProductDisplay productDetails={dancer} />

        </>
    );
}

export default Dancer